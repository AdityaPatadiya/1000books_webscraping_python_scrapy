...
class BookspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for book in books:
            relative_url = book.css('h3 a::attr(href)').get()  # this is how we can access the next page link.
            ...
            yield response.follow(book_url, callback=self.parse_book_page, headers={"User_Agent": self.user_agent_list[random.randint(0, len(self.user_agent_list)-1)]})
        ...
        if next_page is not None:
            ...
            yield response.follow(next_page_url, callback=self.parse, headers={"User_Agent": self.user_agent_list[random.randint(0, len(self.user_agent_list)-1)]})
    ...
    def parse_book_page(self, response):
        ...
        book_item['url'] = response.url,  # it is the url that the response contain the page.
